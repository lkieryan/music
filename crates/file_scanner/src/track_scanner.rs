...
impl<'a> TrackScanner<'a> {
    ...
    pub fn scan_in_pool(
        &self,
        tx: Sender<(Option<String>, Result<MediaContent>)>,
        size: f64,
        path: PathBuf,
    ...
    #[tracing::instrument(level = "debug", skip(self, tx_track))]
    pub fn start(&self, tx_track: Sender<(Option<String>, Result<MediaContent>)>) -> Result<usize> {
        tracing::debug!("Satrting scan");
        self.check_dirs()?;

        let file_list = get_files_recursively(self.dir.clone())?;

        let track_list = file_list.file_list;

        let len = track_list.len();

        for (file_path, size) in track_list {
            self.scan_in_pool(tx_track.clone(), size, file_path, None);
        }
