...
fn test_insert_song() {
    ...
    assert!(result[0].song._id.is_some());
...
fn test_remove_songs() {
    ...
    // Get IDs
    let keep_id = songs[0].song._id.clone().unwrap();
    let remove_id = songs[1].song._id.clone().unwrap();
    ...
    assert_eq!(all_songs[0].song._id.as_ref().unwrap(), &keep_id);
...
fn test_analytics() {
    ...
    if let Some((_, play_time)) = song_analytics {
        assert!(*play_time > 0.0, "Play time should be recorded");
    }
